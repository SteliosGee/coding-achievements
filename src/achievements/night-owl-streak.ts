...
function loadData() {
    streakData = loadTracking<StreakData>('night_owl_streak', { ...DEFAULT_STREAK });
}

function saveData() {
    saveTracking('night_owl_streak', streakData);
}
...
function checkNightOwl() {
    ...
    updateUpgradableAchievement(achievements, 'night_owl_streak', streakData.currentStreak, achievementsFilePath, sidebarProvider);
}

export function resetNightOwlStreakTracking() {
